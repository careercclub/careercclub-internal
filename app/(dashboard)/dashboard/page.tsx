...
export default async function DashboardPage() {
  ...
  return <><header className={styles.moduleHeader}><div><div className={styles.breadcrumb}>Operations</div><h1><i className="ti ti-layout-dashboard" aria-hidden="true" />Dashboard</h1><p>Live operational totals and the latest persisted activity from PostgreSQL.</p></div></header><section className={styles.statsGrid} aria-label="Operational totals">{metrics.map((metric) => <article className={styles.statCard} key={metric.label}><div className={styles.statLabel}>{metric.label}</div><div className={styles.statValue}>{metric.value.toLocaleString()}</div></article>)}</section><section className={styles.card}><DashboardCalendar tickets={tickets} tasks={tasks} events={events} /></section><section className={styles.card}><div className={styles.cardHeader}><div><span>PostgreSQL</span><h2>Recent activity</h2></div><i className="ti ti-history" aria-hidden="true" /></div>{activity.length ? <div className={styles.tableWrap}><table><thead><tr><th>Module</th><th>Action</th><th>Detail</th><th>User</th><th>Time</th></tr></thead><tbody>{activity.map((item) => <tr key={String(item.id)}><td>{item.module || "-"}</td><td>{item.action || "-"}</td><td>{item.detail || "-"}</td><td>{item.user_name || "System"}</td><td>{activityTime(item.created_at)}</td></tr>)}</tbody></table></div> : <p className={styles.mutedCopy}>No persisted activity has been recorded yet.</p>}</section></>;
}
