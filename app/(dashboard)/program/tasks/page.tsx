...
export default async function ProgramTasksPage() {
  return <RecordManager definitionKey="tasks" links={programLinks} rows={await listProgramTasks()} tools={<AiTextRecordParser definitionKey="tasks" kind="task" title="Create task from text" fields={[{ name: "title", label: "Title", required: true }, { name: "description", label: "Description", type: "textarea" }, { name: "status", label: "Status", type: "select", options: ["Todo", "On Progress", "Done"] }, { name: "priority", label: "Priority", type: "select", options: ["High", "Med", "Low"] }, { name: "phase", label: "Phase", type: "select", options: ["Pre Event", "Hari H", "Post Event"] }, { name: "due_date", label: "Deadline", type: "date" }]} /> } />;
}
