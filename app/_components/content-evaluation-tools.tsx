...
function FormatView({ tab, items, query, setQuery, grade, setGrade, action, setAction, select, managementOpen }: { tab: "reel" | "feed"; items: Scored[]; query: string; setQuery: (value: string) => void; grade: string; setGrade: (value: string) => void; action: string; setAction: (value: string) => void; select: (item: Scored) => void; managementOpen: () => void }) {
  const reel = tab === "reel";
  const { pageItems, page, setPage, totalPages } = usePagination(items, 15);
  return <><header className={styles.formatToolbar}><div><input onChange={(event) => setQuery(event.target.value)} placeholder="Cari judul konten..." value={query} /><select onChange={(event) => setGrade(event.target.value)} value={grade}><option value="">Semua Konten</option><option value="top">Top Performer (&gt;=80)</option>{reel ? <><option value="wtr">WTR Tertinggi</option><option value="nfpct">NF% Tertinggi</option></> : <><option value="follows">Follows Terbanyak</option><option value="link">Link Taps Terbanyak</option></>}</select><select onChange={(event) => setAction(event.target.value)} value={action}><option value="">Semua Action</option><option>Boost ke Meta Ads</option><option>Jadiin template konten serupa</option><option>Repurpose jadi format lain</option></select></div><button className={styles.primaryButton} onClick={managementOpen}><i className="ti ti-plus" /> Tambah {reel ? "Reels" : "Feed & Carousel"}</button></header><div className={styles.tableCard}>{items.length ? <><div className={styles.tableScroll}><table><thead><tr><th>Konten</th><th>Score</th><th>Reach</th>{reel ? <><th>WTR</th><th>NF%</th><th>Saves</th><th>Shares</th><th>Follows</th><th>Follow Rate</th><th>Link Taps</th></> : <><th>Saves</th><th>Save Rate</th><th>Shares</th><th>Comments</th><th>Follows</th><th>Link Taps</th><th>NF%</th></>}<th>Action</th></tr></thead><tbody>{pageItems.map((item) => { const followRate = number(item.row.reach) ? number(item.row.follows) / number(item.row.reach) * 100 : 0; return <tr key={String(item.row.id)} onClick={() => select(item)}><td><strong>{String(item.row.judul || "Untitled")}</strong><small>{dateLabel(item.row.tanggal)}</small></td><td><Score score={item.score} short /></td><td><strong>{compact(item.row.reach)}</strong></td>{reel ? <><td>{number(item.row.wtr) ? `${number(item.row.wtr)}%` : "-"}</td><td>{number(item.row.nf_pct)}%</td><td>{compact(item.row.saves)}</td><td>{compact(item.row.shares)}</td><td className={styles.accent}>{compact(item.row.follows)}</td><td>{followRate.toFixed(2)}%</td><td className={styles.accent}>{compact(item.row.link_taps)}</td></> : <><td>{compact(item.row.saves)}</td><td>{percent(item.score.rates.saveRate || 0)}</td><td>{compact(item.row.shares)}</td><td>{compact(item.row.comments)}</td><td className={styles.accent}>{compact(item.row.follows)}</td><td className={styles.accent}>{compact(item.row.link_taps)}</td><td>{number(item.row.nf_pct)}%</td></>}<td>{String(item.row.action || "-")}</td></tr>; })}</tbody></table></div>{totalPages > 1 ? <div className={styles.pagination}><button className={styles.smallButton} disabled={page <= 0} onClick={() => setPage(Math.max(0, page - 1))} type="button"><i className="ti ti-chevron-left" /></button><span>Page {page + 1} / {totalPages}</span><button className={styles.smallButton} disabled={page >= totalPages - 1} onClick={() => setPage(Math.min(totalPages - 1, page + 1))} type="button"><i className="ti ti-chevron-right" /></button></div> : null}</> : <Empty text="Belum ada konten dalam range ini" />}</div></>;
}
